...
if (isset($_POST['datas']) && is_array($_POST['datas'])) {
  ...
  if ($_POST['selection'] === 'delete') {
    ...
    foreach ($datas as $data) {
      ...
      $resData = koibumiDeleteData::deleteData($day, $num, 'default');
      $favfilename = dirname(__FILE__). '/../../datas/fav/'.$day.'.csv';

      if(file_exists($favfilename)) {
        $favDatas = adminCsvHandler::openCSV($favfilename);
        $key = array_search($resData, $favDatas);
        if($key !== false) {
          $resData = koibumiDeleteData::deleteData($day, $key, 'fav', $resData);
        }
      }
    }
    ...
    foreach ($datas as $data) {
      ...
      $arr = adminCsvHandler::openCSV($day);
      $fav = $arr[$num];
      $filename = dirname(__FILE__). '/../../datas/fav/'.$day.'.csv';

      if(file_exists($filename)) {
        $favDatas = adminCsvHandler::openCSV($filename);
      ...
      $fp = fopen($filename, 'a');
      $fav[2] = koibumiCheckSendData::doublequotation($fav[2]);
      $fav[4] = koibumiCheckSendData::doublequotation($fav[4]);
      $line = implode(',' , $fav);
      fwrite($fp, $line . "\n");
      fclose($fp);
    }
    ...
    foreach ($datas as $data) {
      ...
      $defDatas = adminCsvHandler::openCSV($day);
      $retData = $defDatas[$num];

      $resData = koibumiDeleteData::deleteData($day, $num, 'fav', $retData);
    }
    ...
    foreach ($datas as $data) {
      ...
      $line = adminCsvHandler::openCSV($day);
      $denyIP = $line[$num][1];
